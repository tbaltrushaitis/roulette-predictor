...
define([
  ...
  , function (
    ...
    function initPlugins () {
        var rangesCanvas    =   $("#chart-steps-freqs").get(0).getContext("2d");

        $(".knob").knob();
        //$("#win_gauge").trigger("configure", winOpts);

        //----------------------------
        //- RANGES FREQUENCIES CHART -
        //----------------------------
        var aLabels =   _.range(stepsLimit + 3)
          , aData   =   _.times(stepsLimit + 3, function (n) {return null;})
          , rangesChartData = {
                "labels":  aLabels
              , "datasets": [
...
/*        $('#win_gauge')
    ...
    function updatePredictors () {
        ...
        // console.log("goWinLast:\t", goWinLast);
        // console.log("curWin / curStep: \t", curWin, "/", curStep);
